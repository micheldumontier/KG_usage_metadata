...
\paragraph{\textbf{Schema-version robustness.}} The coverage above is measured against the 2024 endpoint schema, whereas the 2013 log queried an earlier Bio2RDF release (Release~2, built from 2012--2013 data); 2019 queries, by contrast, hit the same release (Release~4) that the 2024 endpoint still serves, so only the 2013 figure is cross-version. The mismatch is not negligible: of the distinct vocabulary IRIs referenced by 2013 organic queries, only \textbf{17.5\% appear in the 2024 schema}, because datasets were retired and per-dataset vocabularies revised across releases (e.g.\ 38 of the 59 datasets queried in 2013 are absent from the 2024 endpoint). To bound the effect, we recovered the Release-2 schema directly from Bio2RDF's published per-dataset statistics files (157 types and 628 predicates across 21 dataset vocabularies) and recomputed coverage against it. The version-matched schema recognizes \textbf{29.6\%} of the referenced vocabulary (vs.\ 17.5\% for 2024), and 2013 organic coverage rises to \textbf{64.1\%}. The 2024-based figure is therefore a \emph{conservative, cross-version} estimate; aligning the schema to the queried release only \emph{increases} the 2013 coverage, so the equal-effort gap over organic-2019 reported above is, if anything, understated. (Release~2's statistics enumerate all predicates and only instantiated types --- a broader basis than our restrictive 2024 type/predicate extraction --- so the two percentages are not strictly on the same footing; the robustness point is directional, that version alignment raises rather than lowers 2013 coverage.) We retain the 2024-schema figures in Tables~\ref{tab:4} and~\ref{tab:5} for cross-period consistency and treat the Release-2 recomputation as a robustness check; the extracted Release-2 schema is released with our artifacts.
