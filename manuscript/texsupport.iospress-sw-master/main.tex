...
Previous work \cite{mohammadi2024analyzing} reported that most of the schema (types and predicates) of the Bio2RDF KG was referred to within the SPARQL query logs over 30 months of queries. However, this work provided only a superficial view of user interaction with the KG, lacking deeper insights into the extent to which each schema element is accessed by users as well as differentiating between organic and robotic interactions. 
...
\item \textbf{Identification and analysis of usage patterns in query logs.} We compute schema-element frequency distributions and conduct statistical assessments (Shapiro--Wilk, Wilcoxon signed-rank, and Spearman's rank correlation) to evaluate the stability and changes in schema-element usage for organic and robotic queries in Wikidata and Bio2RDF across different time periods. We further introduce measures of how \emph{concentrated} usage is (Gini, evenness, and Pareto statistics; Section~\ref{sec:concentration}), a procedure for relating schema-element introduction to subsequent query adoption, and a predicate co-occurrence grouping that characterises the kinds of queries through which classes are reached (Section~\ref{sec:templates}).
...
Listing~\ref{lst:l2} presents the pseudocode and queries used to extract the schema of a KG. The extraction process involves two main queries. Query~1 (line 7 in Listing~\ref{lst:l2}) retrieves all types, while Query~2 (line 13 in Listing~\ref{lst:l2}) extracts relationships between instances of different types; for Bio2RDF, Query~2 is also how predicates are identified, whereas for Wikidata predicates are identified differently, as described below. Using two optimized queries instead of a single complex query improved performance in large KGs, preventing timeout issues. For a few types, Query~2 still encountered timeouts due to the high number of instances and relationships; in such cases, simplified queries were used to mitigate the issue.
...
The two KGs differ slightly in how predicates are identified. For Bio2RDF, predicates are extracted through Query~2 and therefore correspond to relationships observed between typed resources. For Wikidata, predicates are identified directly from the \texttt{wdt:P} property namespace, while types are identified through \texttt{wdt:P31} and \texttt{wdt:P279}. This reflects the different schema representations used by the two KGs.
...
\subsection{Schema-Element Usage Distributions and Temporal Dynamics}\label{sec:s6}
...
\begin{comment}
    I have a broad reproducibility and traceability concern about the analyses reported in the manuscript. Let me give the Bio2RDF analysis as one concrete example of the current confusion in the repository. The relevant workflow is distributed across several scripts whose roles and relationships are not clear without manually tracing the code. For example, `bio2rdf2013_log_compose.py` classifies the raw 2013--2015 server traffic and extracts the browser-UA queries; `bio2rdf_split_organic_by_release.py` independently reprocesses the raw log to create the Release-2 and Release-3 query subsets used in the schema-version robustness analysis; and `bio2rdf2013_organic_coverage.py` then takes the extracted organic queries and computes their schema coverage. A later script, `bio2rdf_recompute_conformant.py`, appears to have been introduced after the Bio2RDF schema definition was corrected by removing four non-conforming W3C predicates, and it recomputes the affected coverage, singleton, concentration, and rarefaction results under the corrected definition.

The difficulty is that older and corrected analysis logic currently coexist. For example, `bio2rdf2013_organic_coverage.py` still describes a 545-element Bio2RDF schema and prints the older `116/545 = 21.28%` organic-2019 comparison, whereas the current manuscript uses the corrected 541-element schema. Presumably, the later recomputation script provides the corrected results, but this is not evident from the repository itself. Similarly, the release-splitting script produces the R2/R3 query files, but the subsequent steps that recover the historical schemas and produce the reported release-matched vocabulary and coverage figures are not apparent from these files.

I strongly suggest giving each analysis a dedicated README. A useful way to reorganize the repository would be to work through the Results section analysis by analysis, largely following the manuscript subsections, and make the code supporting each analysis clearly identifiable. For each analysis, the repository should state which script or scripts are authoritative, what input files they consume, what intermediate files they produce, and which final outputs correspond to the numbers, tables, or figures reported in the manuscript. Where the final outputs are reasonably small, I would also include them in the repository rather than only documenting how they were generated. Superseded scripts or outputs should be removed from the main analysis path or clearly marked as obsolete. At present, the amount of overlapping, intermediate, and superseded material makes it very difficult, and at times uncertain, to reconstruct the actual workflow used for the current paper.
...
\subsection{Extracting Total Schema Elements}
Table \ref{tab:3} presents the statistics of total schema elements across different KG versions. Initially, the Bio2RDF schema comprised 166,444 unique types. After excluding types that do not adhere to the template \url{http://Bio2RDF.org/DATASET_vocabulary:SCHEMA_ELEMENT}',' 545 types remain. A further refinement, by excluding elements with the template \url{'http://Bio2RDF.org/DATASET_vocabulary:Resource,'} reduces the count to 350 types and 191 predicates, distributed across 26 subgraphs within the Bio2RDF KG on the current endpoint (2024). Out of these 26 subgraphs, only 17 have corresponding query logs available in the Bio2RDF log2013 dataset, as depicted in Figure \ref{fig:l3}. These 17 subgraphs contain 270 types and 125 predicates, in total 395 schema elements.

As represented in Table \ref{tab:3}, the Wikidata KG version 2017 contains 103,355 types, whereas the Wikidata KG version 2018 contains 97,445 types, reflecting a reduction in the number of schema types. These results illustrate the variability in schema complexity with Wikidata exhibiting a larger and more diverse set of schema elements compared to Bio2RDF.
\begin{table}[ht]
\centering
...
\hline
Total number of distinct Elements & 395 & 541 & 104,286 & 98,436 \\
Number of distinct Types & 270 & 350 & 103,355 & 97,445 \\
Number of distinct Predicates & 125 & 191 & 931 & 991 \\
\hline
\end{tabular}
}
\end{table}
...
\begin{comment}
Please revisit the definition and extraction of predicates in the revised schema pipeline. I think an important distinction has become blurred in the current version of the paper: an RDF predicate in general is simply the property in a triple $(s,p,o)$, but the paper's notion of a \emph{schema predicate} is narrower. In the original submitted version, the schema was defined through types/classes and the predicates that connect resources belonging to those types. In other words, the intended schema predicates are relationships observed between resources that are associated with identified schema types, rather than every RDF property occurring anywhere in the KG.

This original definition is also the more coherent one for the analysis in this paper, and it is still reflected in the schema-extraction pseudocode currently included in the manuscript. Query~2 first requires a resource to belong to a subject type, follows a predicate \texttt{?p}, and then requires the object to belong to an identified object type. The original Wikidata and Bio2RDF extraction code follows this class--predicate--class pattern.

The newer Wikidata extractor appears to preserve the type definition, since types are still derived from objects of \texttt{P31}/\texttt{P279}, but the predicate extraction no longer appears to implement Query~2. Instead, it seems to include a \texttt{wdt:P...} predicate whenever its value contains a Wikidata Q-item, without requiring the subject and object to satisfy the typing conditions used in the schema definition. This therefore creates a broader predicate universe than the one defined by the original methodology and by the pseudocode still shown in the paper.

Please restore a consistent definition across the manuscript and code. In particular, the introductory definition that currently states that ``a predicate is the property in a triple $(s,p,o)$'' should be revised so that it distinguishes the general RDF notion of predicate from the narrower \emph{schema predicate} used operationally in this paper. The schema-predicate definition, pseudocode, extraction scripts, and reported schema sizes should all reflect the same class--predicate--class criterion.

If the broader predicate extraction in the newer code was introduced intentionally, please explain the methodological reason for changing the schema definition from the original submitted version and document that change explicitly in the response. Otherwise, please bring the revised extractor back into alignment with the original definition and recompute any affected schema sizes and downstream results.
\end{comment}


\begin{comment}
I am also confused about which schema-extraction output is actually being used for the Wikidata statistics reported in Table~2. In the original submitted version of the paper, Table~2 reported 934 predicates for Wikidata 2017 and 992 predicates for Wikidata 2018, together with the corresponding type counts. In the current revised manuscript, Table~2 still reports the same Wikidata schema counts.

Given that the schema-extraction code appears to have been substantially revised and a new Wikidata extractor has been introduced, could you please verify which files/scripts actually produced the counts currently reported in Table~2? If the revised extraction methodology is genuinely different from the original one, it is surprising that the schema counts remain exactly unchanged. Please check whether these numbers were recomputed using the current pipeline or simply carried over from the original manuscript, and make sure the table reflects the schema produced by the final extraction method used in the paper.
\end{comment}
...
Table \ref{tab:4} presents the statistics of used schema elements in various datasets, detailing the total number of distinct elements, types, and predicates identified in the queries. From Bio2RDF log2019\_kg2024, 525 distinct elements were extracted, comprising 334 distinct types and 191 distinct predicates. In comparison, Bio2RDF log2013\_kg2024 contains 313 distinct elements, with 227 distinct types and 86 predicates. The robotic queries for Bio2RDF log2019 match the overall log2019\_kg2024 dataset, both reporting 525 distinct elements, 334 classes, and 191 predicates, while the organic queries yield a much smaller subset of 113 elements, 58 types, and 55 predicates.
...
\begin{figure}
    ...
    \caption{Individual-based rarefaction curves (expected distinct schema elements vs.\ schema-element occurrences sampled, both axes log-scaled) for organic (green) and robotic (red) logs. The dotted line marks the maximum common sampling effort for the organic and robotic logs, corresponding to the size of the organic log. In Bio2RDF the two curves stay close across the shared sampling range, with the organic log slightly richer at the common effort. In Wikidata, organic richness exceeds robotic's across most of the shared range; the curves converge and swap order only near the common effort, where the rarefied robotic log becomes very slightly richer (Table~\ref{tab:rarefaction}).}
...
Controlling for sampling effort substantially reduces the apparent robotic advantage in both KGs, though it is fully removed only for Bio2RDF. For \textbf{Bio2RDF}, at the maximum common sampling effort the robotic log recovers \emph{no} more distinct schema elements than the organic log (99 vs.\ 113; 18.4\% vs.\ 20.9\%). A Monte-Carlo subsampling bootstrap (400 replicates, robotic rarefied to the organic effort of 1{,}220 occurrences) puts the equal-effort ratio at \textbf{0.88$\times$, 95\% CI [0.75, 1.02]}, an interval that contains~1: the two query types reach comparable breadth once effort is equalized, and the large raw gap in Table~\ref{tab:5} (97\% vs.\ 21\%) is a volume effect. For \textbf{Wikidata}, the picture reverses: at the maximum common sampling effort, the same bootstrap gives a ratio of only \textbf{1.03$\times$, 95\% CI [1.02, 1.05]} (robotic 13.3\% vs.\ organic 12.8\% of TSE), and across most of the shared sampling range organic queries are substantially richer per occurrence, with the curves crossing again only in the final approach to the common effort (Figure~\ref{fig:rarefaction}, right). The raw coverage gap in Table~\ref{tab:5} (57\% vs.\ 13\%, i.e.\ $4.4\times$) therefore collapses to a $\sim$3\% difference once effort is equalized, which is consistent with that gap being largely attributable to robotic logs being two orders of magnitude larger rather than to broader robotic interest. (With $n\,{=}\,1.28$M the tiny residual is statistically distinguishable from 1.0, but its magnitude, 3\% vs.\ Bio2RDF's 79\%, is negligible.)
...
\begin{comment}
    I think the overall size-controlled comparison is useful, and the rarefaction design is appropriate for testing whether the raw organic/robotic coverage gap is largely a size effect. A few details should nevertheless be clarified.

First, the interpretation should be aligned more closely with the reported equal-effort results. For Bio2RDF, the robotic log is slightly lower at equal effort (0.88×, with the reported interval including 1), whereas for Wikidata the robotic log remains slightly higher (1.03×, with the reported interval [1.02, 1.05]). I therefore suggest softening statements such as “neither shows a robotic breadth advantage at equal effort” or that controlling for effort “removes” the robotic advantage in both KGs. The substantive result seems to be that the very large raw robotic advantage is largely removed after equalizing effort: Bio2RDF becomes comparable/slightly lower, and Wikidata retains only a very small residual advantage.

Second, the figure caption should be checked against the table. It currently states that organic queries show slightly greater richness per occurrence at the common effort, but the Wikidata equal-effort values reported in Table~\ref{tab:rarefaction} go in the opposite direction (approximately 13,846 robotic vs. 13,394 organic).

third, please check that the total schema-element denominators are used consistently across scripts and tables. In particular, the Bio2RDF denominator appears as 541 in one script (rarefaction_size_control.py) and 545 in the Monte-Carlo script(wd_rarefaction_ci.py). Since the equal-effort percentages are small differences, it would be good to ensure that the denominator is fixed consistently throughout.

One small plotting point: the rarefaction curves are evaluated at 40 linearly spaced sample sizes but displayed on a log-scaled x-axis. The figure looks reasonable, but for a clearer representation of the low-effort region—especially where the text discusses relative richness at lower sampling effort—it may be preferable to evaluate the curves at log-spaced or otherwise denser common sample sizes.

I have a broad reproducibility concern about the current the dev branch in GitHub repository. At the moment, it contains a mixture of newer analysis scripts, older scripts and outputs, and intermediate files from earlier stages of the work. In several cases, it is difficult to determine which scripts and input/output files correspond to the analyses actually reported in the latest manuscript. For example, the rarefaction analysis is split across rarefaction_size_control.py and wd_rarefaction_ci.py, and without already knowing the workflow it is not immediately clear which results are produced by which script.

I strongly suggest reorganizing the repository so that the files representing the final analyses used in the current manuscript are clearly identifiable. The example-query decontamination analysis is a good model: it has a dedicated wdqs-examples folder and a README explaining the analysis, inputs, outputs, and how the pieces fit together. A similar structure would make the other analyses much easier to verify. Ideally, obsolete or superseded scripts and outputs should be removed from the dev branch.
...
Restricting to the 115{,}420 organic queries (61{,}628 unique; 50.3\% syntactically valid, the rest is the non-SPARQL web spam noted in Section~\ref{s4}.3), 2013-era organic queries cover \textbf{53.4\% of the 2024 Bio2RDF schema} (167/350 types, 122/191 predicates), exercising the expected biomedical core (\emph{drugbank:Drug}, \emph{omim:Gene}/\emph{Phenotype}, \emph{drugbank:Target}, \emph{kegg:Pathway}, \emph{pharmgkb:Disease}). This is far above the 20.9\% of organic-2019, but the comparison must control for effort: the 2013 organic log has roughly five times as many unique queries. Rarefying both to equal sampling effort (Section~\ref{sec:rarefaction}), organic-2013 recovers 26.7\% vs.\ organic-2019's 20.9\%, a residual \textbf{1.28$\times$} difference that persists after controlling for sampling effort. One possible explanation is architectural: in 2013 Bio2RDF exposed a \emph{separate endpoint per dataset}, so human exploration may have spread across the specialized subgraphs, whereas the single unified 2019 endpoint may have concentrated organic queries on a narrower core. We cannot test this directly, since the endpoint architecture and the six-year time gap change together.
...
\begin{comment}
 The code and data files presented in the dev branch should correspond to the current version of the paper. I noticed this particularly in the \texttt{KG Usage Metadata/KG Schema Extractors} directory, where files from the original submitted version remain alongside newer scripts introduced during the revision. For example, the older \texttt{schema generator Wikidata.py} is still present, while the newer \texttt{WD Schema Extract.py} appears to have been introduced for the revised Wikidata schema-extraction pipeline. For someone trying to reproduce the paper, it is currently very difficult to determine which of these files actually produced the schema used in the reported analyses.

 I suggest removing from the dev branch files that are no longer part of the current analysis pipeline, rather than retaining multiple obsolete and current versions together. This should not be limited to the schema-extraction directory: please check the repository more generally and make sure that the scripts, intermediate/output files, CSV files, and other analysis files presented there correspond to the current version of the paper.

 please provide a README for each analysis (or analysis directory) explaining the current pipeline: which script(s) should be run, which input dataset/files they use, which intermediate files they produce, and which table, figure, or result in the manuscript they generate. The README provided for the query-contamination analysis (dev/generated-usage-metadata/wdqs-examples) is a very useful example of the level of documentation I have in mind. Similar documentation for the other analyses would make the revised repository substantially easier to understand and reproduce.

 There is also a specific consistency issue to check in the Bio2RDF schema extraction. The historical-release extractor, \texttt{bio2rdf_release_schema.py}, which constructs release-specific schemas from the published \texttt{*-statistics.nt.gz} files, explicitly removes classes whose local name is \texttt{Resource} (i.e., \texttt{*_vocabulary:Resource}). In contrast, the separate SPARQL-based schema-extraction code used for the Bio2RDF schema underlying the 2019 analysis appears to retain these classes: it filters classes only by the presence of \texttt{_vocabulary} and does not exclude \texttt{*_vocabulary:Resource}. Could you please check why the two extraction pipelines use different definitions? If these generic \texttt{Resource} classes are intended to be excluded from the Bio2RDF schema, this exclusion should be applied consistently to every Bio2RDF schema version used in the paper. Please verify the resulting schema sizes and recompute any affected coverage results if necessary.
\end{comment}


\begin{comment}
    This paragraph (which presents Figure \ref{fig:l5}) feels somewhat abrupt in its current position, since it shifts from reporting the analysis results to describing an interactive visualization. I wonder whether the visualization would fit better in the \emph{Usage Metadata Utility} subsection and be framed as a small utility demonstration. In particular, it shows how usage metadata can support schema exploration: a user trying to understand the KG or formulate a query can inspect not only the schema structure, but also which classes and predicates are frequently used, rarely used, or unused. This could be presented as a concrete example of a usage-aware interface that complements conventional schema documentation or browsing tools.
\end{comment}

\begin{comment}
I suggest either removing this experiment (regarding Pairwise Schema Type Usage of Bio2RDF subsection) or revising the method. The current comparison may not measure the same type of cross-dataset usage in 2013 and 2019: in 2013, Bio2RDF datasets were accessed through separate endpoints, so cross-dataset queries could be expressed using `SERVICE`, whereas in 2019 they could be written as ordinary joins over a unified endpoint. The current method, which only detects cross-subgraph schema types within triple patterns, may therefore miss genuine federated queries in 2013 and make the 3-vs-22 comparison misleading. If the experiment is retained, the 2013 logs should also be analyzed for `SERVICE`-based joins, and the three currently detected 2013 cases should be inspected to determine what they actually represent.
\end{comment}
...
\begin{comment}
   The current definition of class-position usage appears incomplete and should be revised for both Wikidata and DBpedia. As currently defined, Wikidata classes are counted when they occur in object position of \texttt{P31} (\emph{instance of}) or \texttt{P279} (\emph{subclass of}), while DBpedia classes are counted when they occur in object position of \texttt{rdf:type} or \texttt{rdfs:subClassOf}. However, in the subclass relations, the subject is also a class. Therefore, occurrences of Wikidata schema classes as the subject of \texttt{P279}, and of DBpedia schema classes as the subject of \texttt{rdfs:subClassOf}, should also be included in the class-position definition. The methodology should be updated accordingly, and the corresponding analyses should be recomputed. The numerical results may remain similar if most of these classes also occur in the positions already captured by the current definition, but the reported results should be based on the corrected methodology.
\end{comment}


Table~\ref{tab:classvalue} shows the split. Among the entities our extraction counts as Wikidata \emph{used types} (those referenced in queries that are also objects of \texttt{wdt:P31}/\texttt{wdt:P279} in the KG), only \textbf{72.7\% of organic} ones are ever used as a class, \textbf{27.3\% are referenced solely as values}, whereas robotic queries use 94.9\% of their counted types as classes. Weighting by reference volume rather than by distinct element does not narrow this contrast; it inverts it, and the inversion is the more informative result. Counting references rather than elements, \textbf{90.9\% of organic type-item references are in class position} (2.79M of 3.07M) against \textbf{61.5\% of robotic ones} (6.27M of 10.19M). So on the set-level view robotic queries look the more class-oriented of the two (94.9\% vs 72.7\%), while on the volume-weighted view organic queries do (90.9\% vs 61.5\%).

Both views are correct and they measure different things. For organic queries the value-only items are numerous but individually rare: they are 27.3\% of the distinct type-items a human touches yet draw under a tenth of the references to type-items. Human value-position usage is thus a long tail of one-off mentions. Robotic traffic shows the opposite profile: few distinct type-items are value-only (5.1\%), but those that are get referenced in enormous volume, so nearly two fifths of robotic type-item references sit in value position.

This matters for how the over-counting should be understood. Because coverage is a breadth measure (Section~\ref{sec:concentration}), it is the \emph{set-level} figure that determines how much a coverage percentage is inflated, and that inflation is worst for human queries. The volume-weighted figures do not correct the coverage metric; they show that the two query types arrive at their schema usage by different routes, which the single set-level contrast conceals. These results have two implications. First, the published Wikidata coverage \emph{over-counts schema usage}, most so for human queries, where roughly a third of the ``used types'' are mis-counted value-items (\emph{actor}, \emph{female}, \emph{India}). Second, the class/value split differs systematically between the two query types in these logs, though the direction depends on whether one counts elements or references: a larger \emph{share of the distinct type-items} organic queries touch is value-only (27.3\% vs 5.1\%), whereas a larger \emph{share of the references} robotic queries make to type-items is value-position (38.5\% vs 9.1\%). We report this as a difference between the analysed logs; establishing that it reflects querying behavior in general, rather than characteristics of this particular sample of clients and time period, would require further evidence.
...
\hline
Wikidata organic 2017 & 12,773 & 9,286 (72.7\%) & 3,487 (27.3\%) \\
Wikidata robotic 2017 & 58,689 & 55,698 (94.9\%) & 2,991 (5.1\%) \\
\hline
\end{tabular}
\end{table}

\subsubsection{Query Demand and KG Supply}
In Bio2RDF, the union of the 2019 queries reaches 97\% of the schema (Table~\ref{tab:5}), leaving only 16 of 541 schema elements unqueried. In Wikidata, by contrast, organic queries reach only about 4\% of the class universe. This raises an important question: are the few classes that users query also the classes that contain most of the KG's instances?

Schema coverage alone cannot answer this question. It tells us how much of the schema is reached, but not whether the queried classes correspond to the most highly populated parts of the KG. A low coverage value may be less concerning if queries are concentrated on classes that contain a large share of the data. We therefore compare query \emph{demand} with KG \emph{supply}, where supply is measured by the number of instances per class extracted from the dump. We perform this analysis for Wikidata, and later for DBpedia, where low class coverage makes this question particularly relevant.


The answer is a pronounced inversion (Figure~\ref{fig:mismatch}). The most heavily instantiated Wikidata classes are almost never queried: e.g.\ \emph{ravine} ($\sim$17{,}000 instances), \emph{reef}, \emph{cairn}, \emph{qanat}, \emph{shoal}, and (for bots) \emph{Wikimedia navigational template} ($\sim$27{,}000), \emph{protein domain}, and \emph{microRNA}, all bulk- or bot-imported categories with essentially zero human or robotic class-position demand. Conversely, the genuinely class-queried core is small and intuitive: \emph{human} (\texttt{Q5}), \emph{city}, \emph{film}, \emph{country}, \emph{painting} for organic queries, and \emph{human-geographic territorial entity}, \emph{film}, \emph{album}, \emph{church building}, \emph{book}, \emph{business} for robotic. In other words, the bulk of what Wikidata \emph{contains} is not what users \emph{query}, and much of what users query as ``types'' the model does not treat as classes at all.
...
\paragraph{\textbf{DBpedia as an Additional Case Study.}} The preceding analysis shows that a substantial fraction of the schema elements referenced in Wikidata queries are not used in class position. We therefore extend the same analysis to DBpedia to examine whether this pattern is also observed in another large, general-purpose KG. DBpedia provides an informative additional setting because, while it is broad and large like Wikidata, it follows a more conventional OWL-style schema in which classes are represented by dedicated ontology IRIs (\texttt{dbo:}). Across 4.26M distinct executed queries, 722 \texttt{dbo:} classes are referenced. Of these, \textbf{675 (93.5\%) are used in class position}, while only \textbf{47 (6.5\%) occur exclusively outside class position}. This value-only fraction is substantially lower than the 29.8\% observed for organic Wikidata queries and is closer to the near-zero fraction observed for Bio2RDF (Table~\ref{tab:generality}). Across the three KGs, value-only class references are therefore considerably more common in Wikidata than in the two OWL-style KGs. This suggests that the way classes are represented in the underlying data model may contribute to the difference. We also examine the relation between class population and query demand in DBpedia. As in Wikidata, highly populated classes are not necessarily the ones most frequently queried in class position. Some of DBpedia's most heavily instantiated classes, including \emph{Tenure} ($\sim$1.3M instances), \emph{WikimediaTemplate} ($\sim$223k), \emph{MilitaryService}, and \emph{Academic}, receive little or no class-position demand, while queries are concentrated on classes such as \emph{Place}, \emph{Airport}, \emph{Person}, \emph{MusicalArtist}, \emph{PopulatedPlace}, and \emph{Film}. Thus, the mismatch between class population and query demand is also observed beyond Wikidata.
...
Wikidata 2017 (organic) & item-based & 9,286 (72.7\%) & 3,487 (27.3\%) \\
\hline
\end{tabular}
\end{table}
...
\subsection{Schema-Element Usage Distributions and Temporal Dynamics}
...
\begin{figure}
    ...
    \caption{Frequency distribution of schema elements across datasets (A–M). The datasets include: A) Bio2RDF All log2013\_kg2024, B) Bio2RDF All log2019\_kg2024,  C) Bio2RDF robotic log2019\_kg2024, D) Bio2RDF organic log2019\_kg2024, E) Wikidata organic log2017\_kg2017, F) Wikidata organic log2017\_kg2018,  G) Wikidata robotic log2017\_kg2017, H) Wikidata organic log2017\_kg2017, L) Wikidata robotic log2018\_kg2018, M) Wikidata organic log2018\_kg2018. The plot's color represents it's log type: blue for all logs, red for robotic logs, and green for organic logs. Within each KG, all panels share the same x- and y-axis limits, so that differences in peak usage magnitude across query types and time periods are directly comparable; Bio2RDF and Wikidata use separate scales. }
     \label{fig:l6}
\end{figure}

\begin{comment}
    in Figure 9 (\ref{fig:l6}), for clearer comparison, please use the same x- axis and Y- axis scales across all plots belonging to the same KG. Bio2RDF and Wikidata can have different scales because their usage frequencies differ substantially, but within each KG the corresponding panels should share the same Y-axis limits. Currently, the panels are independently scaled, which makes comparisons across query types and time periods difficult and can visually obscure differences in magnitude.

\end{comment}

\subsubsection{Frequency Distribution of Schema Elements}
As described in Section \ref{sec:s61}, we analyze schema element usage patterns by visualizing the frequency distribution of elements across various intervals of robotic and organic query logs from Bio2RDF and Wikidata. Figure \ref{fig:l6} illustrates this distribution on logarithmic axes: the y-axis is the normalized monthly usage count of each schema element and the x-axis is the schema-element rank (most- to least-frequently used). The Bio2RDF contain 541 schema elements, whereas Wikidata 2017 and 2018 include 104,286 and 98,436 elements, respectively. The color scheme distinguishes log types: green represents organic logs, red represents robotic logs, and blue represents all logs (organic and robotic combined). The plots (A–M) exhibit a consistent trend: a sharp decline in schema element usage frequency, suggesting that most schema elements are infrequently used.

Plot A (Bio2RDF All log2013\_kg2024) exhibits a more gradual decline in schema element usage compared to Plot B (Bio2RDF All log2019\_kg2024). Plot C (Bio2RDF robotic log2019\_kg2024) demonstrates a far higher maximum frequency, its most-used element reaching about $2.5\times10^{3}$ uses per month, whereas Plot D (Bio2RDF organic log2019\_kg2024) peaks at roughly $5$ uses per month, nearly three orders of magnitude lower. This contrast highlights the greater intensity of schema element usage in Bio2RDF robotic queries.

Plots E and F (Wikidata All organic log2017\_kg2017 and Wikidata All organic log2017\_kg2018) show a similar distribution of schema element usage across two different versions of Wikidata. Both intervals of robotic logs in Wikidata, Plot G and Plot L (Wikidata robotic log2017\_kg2017 and Wikidata robotic log2018\_kg2018), exhibit steep declines in schema element usage, with maximum frequencies of about $1.8\times10^{6}$ and $2.0\times10^{6}$ uses per month. In contrast, Plot H (Wikidata organic log2017\_kg2017) and Plot M (Wikidata organic log2018\_kg2018) peak an order of magnitude lower, around $4$--$6\times10^{4}$, indicating less intensive schema element usage in organic queries.
...
Usage is highly concentrated in almost every dataset: the used-element Gini ranges from 0.91 to 0.99 and evenness $J$ from 0.31 to 0.56 across the Wikidata logs and the two full Bio2RDF logs, far below the uniform value of 1. The $P_{80}$ column makes the long tail concrete: on the full Bio2RDF logs about $1\%$ of used elements carry 80\% of all references, and on the much larger Wikidata robotic logs the figure is below 0.1\% (a few dozen of $\sim$60{,}000); the ten most-used elements of the full Bio2RDF logs capture 70--91\% of references.

\emph{Bio2RDF-organic-2019} is a clear exception. Its used-element Gini is 0.662 with $J=0.821$, and 27\% of its used elements account for 80\% of references, indicating a comparatively more even distribution of demand among the elements that are actually used. However, this result should be interpreted together with coverage: the log reaches only 113 of the 541 schema elements, and only 13\% of its queries reference any schema element. Thus, the relatively low used-element Gini describes usage within a small subset of the schema rather than even usage across the schema as a whole. When the 428 untouched elements are included as zeros, the full-universe Gini increases to $Gini^*=0.930$. This illustrates why concentration and coverage should be considered together: a relatively even distribution among used elements can coexist with very limited schema coverage.

Because Gini and $J$ are sensitive to $N$, these figures are meant for \emph{within-KG} comparison (across query types and time), like coverage itself, not as a cross-KG ranking of a $N{=}113$ against a $N{=}60{,}000$ distribution. Table~\ref{tab:concentration} also reports Gini and $J$ over the \emph{full schema universe} (the unused elements enter as zeros, so the basis is the same $TSE$ as coverage); this is the stricter reading, since coverage concerns exactly that unused mass. Full-universe Gini is higher still (0.95--0.996), and it is where the sparsest logs move most: Bio2RDF-organic-2019 rises from 0.90 to 0.98 and Wikidata organic from 0.96 to 0.99, because most of the schema is never touched. A high coverage figure such as Bio2RDF's 97\% must therefore be read as ``almost every element is touched at least once,'' \emph{not} as ``demand is spread across the schema'', the two readings are nearly opposite. Coverage and concentration are thus complementary: the former measures how much of the schema is reached, the latter how unequally that reach is distributed, and a faithful usage report needs both. The slightly higher used-element evenness of the Wikidata organic logs ($J \approx 0.51$) than the robotic ones ($J \approx 0.31$--$0.40$) is consistent with the singleton-dominated robotic tail of Section~\ref{sec:rarefaction}.
...
\textbf{Bio2RDF organic-2019} & 113 & 541 & \textbf{0.662} & \textbf{0.821} & 44.7\% & \textbf{27.43}\% & 0.930 / 0.616 \\
...
\subsubsection{Adoption of Newly Introduced Predicates}\label{sec:adoption}

The analyses above examine how the usage of schema elements changes over time, but they do not distinguish newly introduced elements from those that have been available for longer. This distinction is important when usage is used to guide documentation or interface support, since newly introduced elements may not yet have reached the same level of usage as established ones. We therefore ask to what extent newly added properties are used relative to established ones.

To examine this, we identify schema elements that appear in a newer KG version but not in the earlier one, and then examine whether and how prominently they are referenced in the corresponding query logs. For Wikidata, we lack property-creation timestamps, so we use as a proxy the 68 predicates present in our 2018 extracted schema but absent from the 2017 one. Wikidata property identifiers are allocated sequentially, which allows us to further assess this proxy. The highest identifier in the 2017 extraction is \texttt{P4151}; 56 of the 68 candidates (82\%) have higher identifiers, while the remaining 12 are older properties that were absent from the 2017 extraction. We therefore report the results for both the full set of 68 candidates and the subset of 56 higher-identifier properties. We restrict this analysis to predicates because the recency proxy above relies on Wikidata's sequential, property-only identifier namespace; item identifiers (\texttt{Q}-numbers) are shared by all Wikidata entities, not only those used as types, so a type's absence from the 2017 extraction cannot be distinguished from an existing item that was simply not yet used as a type, and no equivalent recency check is available.

Among all 68 candidate properties, 66\% are referenced at least once by robotic queries in 2018 and 28\% by organic queries. For the 56 higher-identifier properties, the corresponding values are similar, at 64\% and 27\%, respectively. Although many of the newly appearing properties are therefore referenced at least once, none occurs among the 100 most frequently queried predicates in the 2018 logs. The highest-ranked candidate is 216th among 934 predicates for robotic queries and 161st among 549 predicates for organic queries. Thus, newly appearing properties are used in queries, but they do not reach the most frequently used part of the predicate distribution during the observed period.

Bio2RDF does not provide a clean setting for the same analysis with the schema snapshots available here. Release~3 introduced 264 types and 1{,}357 predicates, but much of this vocabulary, particularly the predicates, is no longer present in the 2024 schema used in our analysis: 91.5\% of the added predicates and 16.5\% of the added types are absent. Consequently, comparing the Release~3 additions with the 2019 logs using the 2024 schema would confound adoption with subsequent schema changes. Moreover, among the Release~3 elements that remain in the 2024 schema, all 213 types and all 115 predicates are referenced in the 2019 log. Given the already high overall coverage of the Bio2RDF 2019 robotic log (97\%), this subset provides little variation for examining adoption. We therefore restrict the adoption analysis to Wikidata.

\begin{comment}
    the tilie "Adoption of Newly Introduced Schema Elements" sugessts that we are analysing schema elements which includes both types and predicates. however the analysis only investigates the predicates. why types adoption is not investigated should be clarified in this paragraph. 
\end{comment}
...
\begin{comment}
    in the caption of table(tab:temporaltraj) it is mentioned "the fixed 2017 schema universe (TSE${=}104{,}289$)" but in the table 2 of the manuscript the TSE is 104,314. I am wondering why there is a mismatch.
\end{comment}


Three findings emerge (Table~\ref{tab:temporaltraj}). First, \emph{coverage is remarkably stable}, between 3.68\% and 4.63\%, even though query volume ranges from 192K to 896K across the windows; coverage remains within a narrow range despite the large variation in query volume. Second, \emph{rank stability is consistent across the whole trajectory}: the successive-window Spearman correlation of type frequencies — computed over the types observed in both compared windows, not the full fixed 2017 universe — stays in a narrow band ($\rho = 0.52$--$0.59$ for all six adjacent pairs), decaying only mildly to $\rho = 0.43$ over the full $\sim$9-month endpoint gap. The moderate organic stability we reported for the single 2017/2018 pair is therefore not an artifact of those two endpoints, it holds uniformly window-to-window, with gradual long-range drift. Third, the Wilcoxon signed-rank test on per-type frequency change — computed, like the Spearman correlation above, over the types common to each compared window pair — is significant for five of the six adjacent windows ($p$ from $9{\times}10^{-15}$ to $0.037$; only the December--January pair is not, at $p = 0.23$). We do not read this as evidence of instability, for the reason given in Section~\ref{s4}.9: with 1{,}500--1{,}900 paired types per comparison, the test detects arbitrarily small systematic differences, so it establishes only that usage is not perfectly stationary. The effect-oriented measures are the informative ones here, and they point the other way: coverage varies by less than a percentage point across the seven windows, and the rank correlation stays within $\rho = 0.52$--$0.59$. Finally, fifteen types appear in the top-50 of \emph{every} window: \emph{human}, \emph{city}, \emph{film}, \emph{country}, \emph{book}, \emph{year}, \emph{human settlement}, \emph{painting}, \emph{business enterprise}, \emph{television film}, \emph{scientific article}, \emph{cat}, \emph{male}, \emph{female}, and \emph{politician}, a far stronger demonstration of a persistent ``core'' than the pairwise top-50 overlaps above. Notably, two core members (\emph{male}, \emph{female}) are value-position items (Section~\ref{sec:classvalue}), reinforcing that the human-query ``type'' core is partly composed of items the model does not treat as classes.

\begin{comment}

  I think a few details of this temporal analysis should be clarified. First, the seven 28-day windows used by `wd_temporal.py` are not all consecutive. For example, after the 2017-08-07--09-03 window, the next analyzed window begins on 2017-12-03. I do not think this is a problem for the analysis itself, but “successive windows” would be more accurate than “consecutive windows.” The corresponding wording should also be adjusted where the text refers to “all six adjacent pairs” or to the pattern holding “uniformly window-to-window,” since some successive analyzed windows are separated by gaps.

 The Wilcoxon and Spearman calculations also retain only the intersection of types observed in both compared windows (`set(a) & set(b)`). Thus, types that appear in one window but not the other are excluded from these comparisons. This is worth making explicit, particularly because the subsection emphasizes that the schema universe is fixed to the 2017 dump. As currently implemented, the reported Spearman values measure rank correlation among types observed in both compared windows, rather than correlation over the complete fixed type universe. Please check whether this restriction to common types is intentional; if so, the text should state it explicitly.

  I would also slightly soften the interpretation of the coverage result. The observation that coverage remains between 3.68% and 4.63% despite substantial differences in query volume is interesting in itself. However, “More queries in a window do not expand coverage” sounds stronger than what these seven observational windows directly establish. I would instead state descriptively that coverage remains within a narrow range despite the large variation in query volume.

\end{comment}

\subsubsection{Which Queries Reach Rare Classes}\label{sec:templates}
Coverage and frequency tell us which schema elements are used, but not the query contexts in which they occur. To examine these contexts, we group queries according to their \emph{predicate co-occurrence signature}: the sorted set of schema predicates referenced by a query, including predicates occurring in property paths. Queries that use the same set of schema predicates therefore belong to the same group.
...
\subsubsection{Utility: Downstream Applications of Usage Metadata}\label{sec:utility}
The usage metadata generated by our method may be useful in applications such as type autocompletion in a query builder. To demonstrate this potential utility, we examine whether class usage observed in an earlier query log is informative of class demand in a later log.


To examine this, we rank Wikidata classes by how frequently they occur in class position in the \emph{early-2017} organic query logs, and evaluate how well this ranking captures the class demand observed in the \emph{2018} organic log, approximately eight months later. Specifically, we measure what fraction of the later class demand is covered by the top-$k$ classes in the earlier-usage ranking. As in the preceding frequency analyses (Section~\ref{s4}), frequencies are computed over unique (deduplicated) queries so that heavily repeated queries do not dominate the evaluation.
We also compare the usage-based ranking with a simple baseline that does not use query-log information. For this baseline, we rank the same classes by their number of instances in the 2017 Wikidata dump. This provides a readily available content-based ranking derived from the KG itself and allows us to assess whether usage in the earlier log is more informative of later class demand than class size alone.

This use of class ranking is related to prior work on SPARQL autocompletion. Bast et al.~\cite{bast2022autocompletion} study ranked context-sensitive and context-agnostic autocompletion on Wikidata, while de la Parra and Hogan~\cite{delaparra2021autocompletion} derive suggestions using KG structure and content-based ranking signals. Both rank against the KG (and, in the context-sensitive case, against the partially written query). Our experiment focuses specifically on whether query-log usage can provide an additional signal for prioritizing classes. Incorporating such usage information into a full autocompletion system and evaluating it together with context-sensitive and other KG-derived signals remains future work.
...
Figure~\ref{fig:utility} and Table~\ref{tab:utility} present the results of this experiment. The top 50 classes in the usage-based ranking cover 54.1\% of the class demand observed in the 2018 organic log, compared with 42.3\% for the top 50 classes in the KG-supply ranking. For the top 1000 classes, the corresponding coverage is 80.2\% and 68.1\%, respectively. The higher coverage achieved by the usage-based ranking is consistent with the relationship between KG content and query demand observed in Section~\ref{sec:classvalue}. Several classes with many instances in the KG, including \emph{scholarly article}, \emph{Wikimedia category}, \emph{taxon}, \emph{disambiguation page}, \emph{Wikimedia template}, \emph{gene}, and \emph{protein}, occur relatively infrequently in the organic query log. In contrast, the usage-based ranking places frequently queried classes such as \emph{human}, \emph{film}, \emph{city}, \emph{country}, \emph{human settlement}, \emph{university}, and \emph{business} near the top.


Top-$k$ coverage considers whether a class appears within the first $k$
positions, but not its position within that set. We therefore also compare the
two rankings using order-sensitive measures. We compute nDCG@$k$, using each
class's deduplicated 2018 demand as graded relevance, and a demand-weighted
mean reciprocal rank,
$\mathrm{MRR}_w = \sum_t (d_t / r_t) / \sum_t d_t$,
where $d_t$ is the 2018 demand for class $t$ and $r_t$ is its rank.
...
The order-sensitive measures show the same pattern. The usage-based ranking
achieves an nDCG@10 of 94.9\%, compared with 44.5\% for the supply-based
ranking, and remains above 92\% through $k=1000$. Demand-weighted MRR is also
higher for the usage-based ranking ($0.338$ versus $0.117$). Thus, in this
temporal comparison, the earlier usage ranking both covers more of the later
demand within its top-$k$ classes and ranks the classes with greater later
demand closer to the top.

Finally, we assess whether the observed coverage difference is sensitive to
the particular set of 2018 queries. Across 2{,}000 query-level bootstrap
resamples of the 185{,}443 deduplicated 2018 queries, the usage-minus-supply
coverage difference remains positive at every evaluated value of $k$. At
top-50, the difference is 11.8 percentage points (95\% CI $[11.2,12.4]$);
at top-1000, it is also 11.8 points (95\% CI $[11.3,12.4]$).

This demonstration is limited to one KG, one schema-element type, and one
temporal comparison. Evaluating whether the same pattern holds across other
KGs, schema-element types, and time periods remains future work.
...
\paragraph{\textbf{Predicate suggestion.}}
The evaluation above ranks types, but the usage metadata covers predicates too, and predicate
suggestion matters at least as much in a query builder. We therefore repeated the experiment over
predicates, with the same train/test split, both rankings restricted to the 931-predicate 2017
schema, and the content-side analogue of instances-per-class, namely triples per predicate in the
2017 dump, likewise restricted to that schema (Table~\ref{tab:predranking}). Usage again wins, but
by a visibly smaller margin than for types, and the margin narrows sharply as the list grows: nDCG@10
is 76.4\% against 62.5\%, and top-10 demand coverage 35.8\% against 26.8\%, while at $k=931$, the
full predicate schema, coverage is \emph{exactly} equal for both rankings (51.3\% vs 51.3\%) --
a mathematical consequence of both rankings including every schema predicate once $k$ reaches schema
size, not itself a finding about ranking quality. Demand-weighted MRR is $0.227$ against $0.200$, a
$1.1\times$ ratio, narrower still than the $2.9\times$ seen for types.

The asymmetry is informative rather than disappointing, though the explanation has two parts. First,
Wikidata's predicate schema is small (931 elements) relative to its class universe (103,355 types;
Section~\ref{s3}.2), so once $k$ approaches schema size both rankings necessarily include the same
candidates and coverage must converge regardless of ranking quality -- this is why the gap narrows
toward $k=931$ specifically, not evidence that usage and content-based ranking are equally informative.
Second, and more informative, coverage saturates well below 100\% for either ranking: at most 51.3\%
of demand-weighted test references are ever reachable once the entire predicate schema is included.
Because the Wikidata predicate schema is defined as the \texttt{wdt:P} namespace only
(Section~\ref{s3}.2), predicate references that use other patterns, such as statement/qualifier
predicates or general RDF vocabulary, fall outside it by construction and cannot be covered by a
schema-restricted ranking regardless of method -- the predicate-side counterpart of the class/value-
position issue examined in Section~\ref{sec:classvalue}, where a non-trivial share of what queries
reference sits outside the definition used for ranking. Even so, at every $k$ short of the full
schema the usage-based ranking recovers meaningfully more of the reachable demand than the
content-based one, indicating that usage metadata carries real predictive signal on the predicate
side too, just a smaller and more quickly saturating one than for classes.

\begin{table}[ht]
\centering
\caption{Predicate-suggestion counterpart to Table~\ref{tab:ranking}, same train/test split, both rankings restricted to the 931-predicate 2017 schema. Predicates are ranked by past usage or by KG supply (triples per predicate). $k$ runs up to 931, the full schema size, rather than a fixed 1000, since the schema itself is smaller than that. The usage advantage is real but smaller than for types, and narrows as $k$ grows, reaching exact coverage parity by construction at $k=931$.}
\label{tab:predranking}
\begin{tabular}{r|r|r|r|r}
\hline
Top-$k$ & nDCG@$k$ (past usage) & nDCG@$k$ (KG supply) & Coverage (usage) & Coverage (supply) \\
\hline
10  & 76.4\% & 62.5\% & 35.8\% & 26.8\% \\
50  & 69.8\% & 62.7\% & 44.5\% & 42.8\% \\
100 & 68.9\% & 62.0\% & 47.7\% & 45.8\% \\
500 & 67.7\% & 61.4\% & 51.2\% & 50.7\% \\
931 & 67.5\% & 61.4\% & 51.3\% & 51.3\% \\
\hline
\multicolumn{5}{l}{$\mathrm{MRR}_w$: past usage $0.227$ \quad KG supply $0.200$ \quad (ratio $1.1\times$)} \\
\hline
\end{tabular}
\end{table}

Beyond predictive ranking, the same usage frequencies can support a documentation and exploration interface, illustrated for Bio2RDF in Figure~\ref{fig:l5}.
...
Figure \ref{fig:l5} illustrates the extent to which schema elements of the Bio2RDF KG are used in the organic queries of 2019. Nodes represent schema types and edges represent predicates linking instances of those types. Both the size and the color of a node encode the same quantity, its usage frequency: nodes grow larger and brighter as they are queried more often, while rarely used nodes are small and dark. Edge width likewise grows with predicate usage, and schema elements that appear in no query are shown in gray. The visualization makes the long tail visible directly: a handful of large, bright hubs (drug, gene, and disease types) concentrate the bulk of organic usage, while most of the schema remains gray and unqueried. The interactive version, with exact per-element values, is available online\footnote{\url{https://marmhm.github.io/Schema-usage-graph/}}. This extends the usage-frequency visualization approach of Bonifati et al.~\cite{bonifati2019navigating}, whose interactive sunburst chart shows Wikidata predicate-usage frequency, to a joint view of types and predicates together, applied here to an OWL-style KG (Bio2RDF) rather than an item-based one.

\begin{comment}
    Please check the candidate universe used in both the type- and predicate-ranking experiments. In both scripts, the usage ranking is constructed only from classes/predicates that are actually observed in the 2017 query logs, whereas the supply ranking is constructed from all classes/predicates listed in the corresponding 2017 supply file. The two rankings therefore do not necessarily contain the same candidate set, so differences in coverage may reflect both ranking quality and differences in which schema elements are included. I suggest using the same candidate universe for both rankings. One option is to use the 2017 schema elements as the common set and assign zero usage to elements not observed in the 2017 logs; alternatively, if the intention is to rank only elements observed in the logs, the supply ranking should be restricted to exactly that same set. This is especially visible in the predicate results: with 934 predicates in the 2017 schema, a top-1000 cutoff should already include all candidates, so the two coverage values should be identical once the candidate sets are aligned. Please check this and recompute the results accordingly.
    Also, the predicate-ranking results report a top-1000 cutoff, although the 2017 schema contains only 934 predicates. Does it make sense to report top-1000 in this case, given that this already includes the entire predicate universe? It may be clearer to use a smaller cutoff or report this point explicitly as coverage over all predicates.
\end{comment}
...
To quantify the overall usage of a KG, we introduced the SPARQL schema coverage metric, which measures the proportion of schema elements explicitly referenced in queries. Raw coverage is substantially higher in robotic than in organic logs (97\% vs.\ 21\% for Bio2RDF and 60\% vs.\ 14\% for Wikidata). However, the robotic logs are also much larger, so these raw differences may reflect the greater number of schema-element occurrences observed simply because of the larger query volume, rather than broader schema usage. We therefore compare robotic and organic logs at matched sampling effort using individual-based rarefaction (Section~4.6). At equal sampling effort, robotic queries do not exhibit greater schema richness in either KG. In Bio2RDF, the estimated robotic-to-organic richness ratio at the maximum common sampling effort is \(0.88\) (95\% CI \([0.75, 1.02]\)). In Wikidata, the two curves are similarly close at the common effort, with organic queries exhibiting greater richness over much of the shared sampling range. Thus, the large differences in raw schema coverage are largely attributable to differences in sampling effort and log volume rather than to robotic queries accessing a broader portion of the schema.

Beyond coverage, we observed a pronounced long-tail distribution in the frequency of schema-element usage in both datasets. While a small number of schema elements are queried very frequently, the majority are used only rarely. We quantify this concentration in Section~\ref{sec:concentration} and Table~\ref{tab:concentration}: the Gini coefficient exceeds 0.91 in every dataset except the very sparse \emph{Bio2RDF-organic-2019} log, and only 0.1--4\% of used elements account for 80\% of all schema-element references. These results show that high schema coverage does not imply that usage is broadly distributed across the schema: many elements may be reached, while most query activity remains concentrated on a small subset. Coverage and usage concentration therefore capture complementary aspects of schema usage; the breadth of elements accessed and the extent to which usage is concentrated among them.
...
\begin{comment}
    The online GitHub repository does not currently contain an out folder, so the wd_closure_anchor_queries.tsv file and the 13 near-root anchor queries are still not available. Could you please add this file to the repository (e.g. under generated-usage-metadata folder)? 
\end{comment}
...
\begin{comment}
     Regarding the parallel-processing issue discussed in D2, I understand that the query-processing pipeline is used across the three KGs. The preprocessing helpers that were previously in the Bio2RDF-specific-sounding parse_validate_bio2rdf2019.py have now been moved to the more appropriately named sparql_log_preprocess.py, and are used by the Bio2RDF, Wikidata, and DBpedia analyses. Separately, the ordering problem identified in D2 occurred in the parallel() logic of the affected analysis scripts: after queries were processed in parallel, the parsed results were not always restored to their original query order before being associated with query-specific information. D2 identifies affected scripts for all three KGs and states that this ordering logic has now been corrected in all of them. Based on D2 and D4, the results that depend only on unweighted aggregation over the unique parsed queries remained unchanged, whereas results that require the parsed output to remain correctly associated with query-specific information, such as execution counts or other metadata, could change and were recomputed. My understanding from the response is therefore that all analyses affected by this query/result misalignment were re-run after the correction, across Bio2RDF, Wikidata, and DBpedia, and that any resulting changes were propagated to the relevant tables, figures, and manuscript text. Could you please confirm that this understanding is correct and that all affected results have been checked and updated where necessary?
\end{comment}
