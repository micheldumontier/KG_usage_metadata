...
\paragraph{\textbf{Basis of the frequency analyses.}}
The frequency-based results in this section (Figures~\ref{fig:l6} and~\ref{fig:l8}, Table~\ref{tab:concentration}, and the Wilcoxon and Spearman statistics) are computed from the released per-element usage-count files. Those files are derived with the corrected preprocessing of Section~\ref{s3}.3 and, since the schema revision of Section~\ref{s3}.2, are restricted to the conforming schema, so every Bio2RDF figure in the paper rests on one consistently defined element set.

One check is worth recording, because the corrected parameter stripping raises the number of \emph{valid} Bio2RDF-2019 queries substantially (Table~\ref{tab:2}) and it is reasonable to ask whether the per-element counts move with it. Re-deriving the usage vector for \emph{Bio2RDF-organic-2019} directly from the corrected valid-query set gives a Spearman rank correlation of $\rho = 0.954$ with the released counts over the 109 shared elements, an identical set of ten most-used elements, and a largest single-element difference of 21 references. Coverage differs by 0.2 percentage points. The reason so large a change in valid-query count leaves the usage vector almost fixed is that 86.9\% of valid organic queries reference no schema element at all (Section~\ref{sec:bio2rdfresults}), so most of what the correction recovers contributes nothing to any element's count.
