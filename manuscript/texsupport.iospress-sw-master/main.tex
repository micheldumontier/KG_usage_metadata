...
Usage is extremely concentrated in every dataset: Gini ranges from 0.90 to 0.99 and evenness $J$ from 0.31 to 0.51, far below the uniform value of 1. The $P_{80}$ column makes the long tail concrete --- on the compact Bio2RDF schema roughly 1\% of used elements (about six) carry 80\% of all references, and on the much larger Wikidata robotic logs the figure is below 0.1\% (a few dozen of $\sim$60{,}000); the ten most-used Bio2RDF elements alone capture 83--93\% of references. A high coverage figure such as Bio2RDF's 97\% must therefore be read as ``almost every element is touched at least once,'' \emph{not} as ``demand is spread across the schema'' --- the two readings are nearly opposite. Coverage and concentration are thus complementary: the former measures how much of the schema is reached, the latter how unequally that reach is distributed, and a faithful usage report needs both. The slightly higher evenness of the Wikidata organic logs ($J \approx 0.51$) than the robotic ones ($J \approx 0.31$--$0.40$) is consistent with the singleton-dominated robotic tail of Section~\ref{sec:rarefaction}.

\begin{table}[ht]
\centering
\caption{Concentration of schema-element usage. $N$ = distinct used elements; Gini and Pielou $J$ are computed over the per-element usage-frequency vector; top-10\% = share of all references captured by the ten most-used elements; $P_{80}$ = fraction of used elements accounting for 80\% of references. Usage is highly concentrated (Gini $\geq 0.90$) in every dataset.}
\label{tab:concentration}
\begin{tabular}{l|r|r|r|r|r}
\hline
Dataset & $N$ & Gini & $J$ & top-10\% & $P_{80}$ \\
\hline
Bio2RDF All-2013 & 317 & 0.955 & 0.382 & 83.2\% & 2.84\% \\
Bio2RDF All-2019 & 529 & 0.952 & 0.386 & 92.8\% & 1.13\% \\
Bio2RDF robotic-2019 & 529 & 0.953 & 0.384 & 93.1\% & 1.13\% \\
Bio2RDF organic-2019 & 116 & 0.901 & 0.333 & 84.7\% & 4.31\% \\
Wikidata robotic-2017 & 59{,}603 & 0.992 & 0.404 & 39.9\% & 0.09\% \\
Wikidata robotic-2018 & 59{,}582 & 0.984 & 0.308 & 74.4\% & 0.02\% \\
Wikidata organic-2017 & 13{,}394 & 0.957 & 0.506 & 52.7\% & 1.08\% \\
Wikidata organic-2018 & 13{,}900 & 0.957 & 0.508 & 52.1\% & 1.09\% \\
\hline
\end{tabular}
\end{table}
...
The rank shifts above raise a complementary question of \emph{adoption dynamics}: when new schema elements are introduced, do they ever become popular? We use the predicate as the clean signal of schema growth, since Wikidata's predicate vocabulary grows monotonically whereas its item-based ``classes'' do not. Sixty-eight predicates are present in the 2018 Wikidata dump but absent from the 2017 dump. They are adopted readily in terms of \emph{presence} --- 71\% are queried at least once by robotic clients in 2018 (31\% by organic users) --- but not in terms of \emph{popularity}: none of the 68 reaches the top 100 most-queried predicates, the best-ranked new predicate sits at rank 212 of 933 (robotic) and 157 of 546 (organic), and the median queried-new predicate falls in the bottom 15--30\% of the rank distribution. Bio2RDF shows the same pattern more starkly across the Release-2$\to$Release-3 upgrade (Section~\ref{sec:bio2013}): of the 1{,}357 predicates Release~3 added, only 9\% are queried at all in the 2019 log and just two reach the top 20 most-used predicates. Newly introduced schema elements are therefore \emph{tried} but rarely \emph{displace the established core} within a year --- the temporal counterpart of the stable-core finding, and a further instance of the content--demand inversion (Section~\ref{sec:classvalue}) in which schema growth far outpaces query adoption.
...
\subsubsection{Which Query Templates Drive Coverage}\label{sec:templates}
Coverage and frequency describe \emph{which} elements are used; a complementary question, in the spirit of the query-template analysis of Asprino et al.~\cite{asprino2023your}, is \emph{through what kinds of queries} they are reached. We assign each query a \emph{template} given by its predicate signature --- the sorted set of schema predicates it references, property-path members included --- so that queries sharing the same predicates form one template family. This is a coarser but fully reproducible relative of the Asprino templates, and it suffices to ask which templates drive coverage and whether rare classes are reached through specialised templates. We compute it on the human-facing logs of both KGs: the Bio2RDF organic-2019 valid uniques (8{,}140 queries) and the pooled Wikidata organic windows (858{,}969 valid uniques).

Query \emph{volume} is dominated by a handful of templates, but schema \emph{breadth} is not. In Bio2RDF, just \textbf{2} of 438 templates account for 80\% of organic queries, and the ten most frequent templates carry 90\% of query volume yet introduce only 75\% of the distinct queried classes; the remaining breadth comes from a long tail of specialised templates (67\% of templates are used by exactly one query). Wikidata's item-based human queries are far more template-diverse --- it takes 3{,}179 of 68{,}169 templates (4.7\%) to cover 80\% of volume, and the top ten carry only 28\% --- but the same volume-versus-breadth split holds. Rare classes are indeed reached through specialised templates: the Spearman correlation between a class's query frequency and the popularity of the most popular template referencing it is positive in both KGs ($\rho = 0.23$ for Bio2RDF, $\rho = 0.34$ for Wikidata), and the gap is stark at the extremes --- the median bottom-decile (rarely queried) class is reached only through a template used once (Bio2RDF) or 22 times (Wikidata), whereas the median top-decile class appears in templates used 3{,}525 and 17{,}472 times respectively. Coverage thus rests on two different query populations: a few high-volume templates that exercise the popular core, and a long tail of specialised, often single-use templates that account for most of the schema breadth. The contrast between the two KGs --- two templates driving most Bio2RDF volume versus thousands for Wikidata --- is itself a signature of modeling style: an OWL-style biomedical schema is queried through a few stereotyped shapes, an item-based encyclopedic KG through a vastly more heterogeneous set.
