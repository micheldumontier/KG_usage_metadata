...
The clearest case is the Bio2RDF Release-2$\to$Release-3 upgrade (mid-2014; Section~\ref{sec:bio2013}), for which the added vocabulary is documented. Of the \textbf{1{,}357 predicates added in Release~3}, only \textbf{9\%} are queried in the 2019 log, and only two appear among the top 20 most-used predicates.

For Wikidata, we do not have property-creation timestamps. We therefore use as a proxy the 68 predicates that are present in our 2018 extracted schema but absent from the 2017 extracted schema. These predicates are new to the schema under our extraction criteria, although this does not necessarily mean that the corresponding Wikidata properties were newly created during this period. Among these predicates, 71\% were queried at least once by robotic clients in 2018 and 31\% by organic users. However, none appeared among the top 100 most-queried predicates, with the highest-ranked predicate at 212/933 for robotic queries and 157/546 for organic queries. Their median ranks were also within the bottom 15--30\% of the ranking.
...
Overall, in both KGs, newly introduced schema elements rarely became widely used within the examined period. This is consistent with the stable-core pattern observed in the previous analyses, where usage remains concentrated around a relatively small set of established elements. A more precise analysis of Wikidata could use property-creation timestamps to determine exactly when each property was introduced, which we leave for future work.
