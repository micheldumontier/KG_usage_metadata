...
These observations reframe the coverage results for Wikidata: a single ``schema coverage'' percentage, borrowed from the OWL-style Bio2RDF setting, conflates class usage with value usage on an item-based KG. A faithful figure restricts to class-position references over instantiated classes; the residual --- and more informative --- story is the structural mismatch between Wikidata's instantiated content and actual query demand. We return to the implications in Section~\ref{s5}.
...
``coverage'' is the more inflated of the two. Residual caveats remain --- \texttt{wdt:P279}
