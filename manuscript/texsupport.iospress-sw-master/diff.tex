...
%DIF ADD main.tex                                           Sun Jun 14 15:18:35 2026
...
\paragraph{\textbf{\DIFadd{Schema-version robustness.}}} \DIFadd{The coverage above is measured against the 2024 endpoint schema, whereas the 2013 log queried an earlier Bio2RDF release (Release~2, built from 2012--2013 data); 2019 queries, by contrast, hit the same release (Release~4) that the 2024 endpoint still serves, so only the 2013 figure is cross-version. The mismatch is not negligible: of the distinct vocabulary IRIs referenced by 2013 organic queries, only }\textbf{\DIFadd{17.5\% appear in the 2024 schema}}\DIFadd{, because datasets were retired and per-dataset vocabularies revised across releases (e.g.\ 38 of the 59 datasets queried in 2013 are absent from the 2024 endpoint). To bound the effect, we recovered the Release-2 schema directly from Bio2RDF's published per-dataset statistics files (157 types and 628 predicates across 21 dataset vocabularies) and recomputed coverage against it. The version-matched schema recognizes }\textbf{\DIFadd{29.6\%}} \DIFadd{of the referenced vocabulary (vs.\ 17.5\% for 2024), and 2013 organic coverage rises to }\textbf{\DIFadd{64.1\%}}\DIFadd{. The 2024-based figure is therefore a }\emph{\DIFadd{conservative, cross-version}} \DIFadd{estimate; aligning the schema to the queried release only }\emph{\DIFadd{increases}} \DIFaddend the \DIFdelbegin \DIFdel{nodes follows a gradient from yellow (least used ) to dark blue (most used )}\DIFdelend \DIFaddbegin \DIFadd{2013 coverage, so the equal-effort gap over organic-2019 reported above is, if anything, understated. (Release~2's statistics enumerate all predicates and only instantiated types --- a broader basis than our restrictive 2024 type/predicate extraction --- so the two percentages are not strictly on the same footing; the robustness point is directional, that version alignment raises rather than lowers 2013 coverage.) We retain the 2024-schema figures in Tables~\ref{tab:4} and~\ref{tab:5} for cross-period consistency and treat the Release-2 recomputation as a robustness check; the extracted Release-2 schema is released with our artifacts.
}
...
\DIFdelbegin \DIFdel{Unused schema elements (both nodes and edges)are displayed in gray. The interactive version of this visualization is available online }\footnote{%DIFDELCMD < \url{https://marmhm.github.io/Schema-usage-graph/ } %%%
}%DIFAUXCMD
