...
Throughout this paper we use three terms consistently. A \emph{type} (or class) is a resource used to classify instances, i.e., the object of a typing predicate such as \texttt{rdf:type}/\texttt{wdt:P31} or a subclass predicate such as \texttt{rdfs:subClassOf}/\texttt{wdt:P279}. A \emph{predicate} is the property in a triple $(s,p,o)$. An \emph{individual} is an instance, i.e., a non-schema resource (for example, a specific drug or person). We refer to the set of distinct types and predicates of a KG as its \emph{schema elements}. Finally, we study \emph{syntactic usage}: a schema element is counted as used when its IRI is explicitly written in a query. We use this term in deliberate contrast to a \emph{semantic} (effective) notion of usage that would additionally credit the elements a query reaches only at evaluation time --- through inference or property-path traversal --- without naming them. We adopt syntactic usage throughout and argue (Sections~\ref{sec:used} and~\ref{s5}) that it is both the appropriate primitive for our motivating applications (documentation, interface design, autocomplete, deprecation) and the only well-behaved one on an item-based KG; characterising semantic usage is future work.
...
Coverage and frequency describe \emph{which} elements are used; a complementary question, related to the query-template analysis of Asprino et al.~\cite{asprino2023your}, is \emph{through what kinds of queries} they are reached. We group queries by their \emph{predicate co-occurrence signature} --- the sorted set of schema predicates they reference, property-path members included --- so that queries sharing the same schema predicates form one group. This is a much coarser notion than an Asprino template (it ignores join shape, variables, filters, and aggregation, and any two queries with the same predicates collapse together); we use it only as a reproducible proxy for ``kind of query,'' and do not claim template equivalence. Two preprocessing points matter. First, we compute this on the human-facing logs of both KGs --- Bio2RDF organic-2019 and the pooled Wikidata organic windows --- but restrict to \emph{schema-bearing} queries (those referencing at least one schema predicate or class), because, as Section~\ref{s4}.5 shows, 86.9\% of the Bio2RDF organic queries reference no schema element at all (endpoint default-sample/probe traffic) and would otherwise collapse into a single meaningless group; the same filter removes 23.1\% of the Wikidata organic queries. This leaves \textbf{1{,}065} schema-bearing Bio2RDF queries and \textbf{660{,}672} Wikidata ones. Second, a large share of the Bio2RDF survivors (61\%) are bare type-assertions (\texttt{?x a Class}) with no schema predicate, which form one high-volume group; Wikidata has none, since its classes are reached through the schema predicates \texttt{wdt:P31}/\texttt{wdt:P279}.

Query volume is concentrated in a modest number of signatures: \textbf{25 of 174} signatures cover 80\% of the Bio2RDF schema-bearing queries, and \textbf{1{,}078 of 31{,}495} (3.4\%) cover 80\% of the Wikidata ones --- with a long tail of single-use signatures (63\% and 42\% of signatures, respectively). Wikidata's item-based human queries are thus far more heterogeneous in their predicate combinations than Bio2RDF's ($31{,}495$ vs.\ $174$ distinct signatures), consistent with a broad encyclopedic schema versus a compact biomedical one. The generic ``instances of a type'' pattern is the single most popular Bio2RDF group and, because each such query names a different class, the ten most popular signatures already touch $86.5\%$ of the distinct queried classes (on $72.7\%$ of the volume); for Wikidata the top ten cover $62.5\%$ of classes on $35.8\%$ of volume.

The substantive finding is that \emph{rare classes are reached through less-popular queries}. The Spearman correlation between a class's query frequency and the popularity of the most popular signature referencing it is positive in both KGs ($\rho = 0.14$ Bio2RDF, $\rho = 0.40$ Wikidata), and the extremes are far apart: the median bottom-decile (rarely queried) class is reached only through a signature used once (Bio2RDF) or 50 times (Wikidata), whereas the median top-decile class appears in a signature used 654 and 54{,}072 times. Because a class's frequency mechanically lower-bounds the popularity of any signature containing it, we checked that this is not an artifact: the most popular signature of a class is \emph{strictly} more popular than the class itself for 84\% (Bio2RDF) and 99\% (Wikidata) of classes, with a median popularity-to-frequency ratio of $109\times$ and $5{,}407\times$ --- i.e.\ popular classes genuinely ride signatures that are popular for many \emph{other} queries too, so the association is not merely the arithmetic floor. Rare classes, by contrast, tend to appear only in rare, specialised queries --- a behavioural counterpart to their low frequency.
